...
#            labels, preds = sess.run([label, prediction], feed_dict=feed)
            preds = sess.run([prediction], feed_dict=feed)
            results = ["{0} For file: {1}, Prediction: {2}".format(datetime.now().isoformat(), l, p) for l, p in zip(batch_ys, preds)]
            print(results)
